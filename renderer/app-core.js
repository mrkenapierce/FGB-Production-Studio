...
function fgbRender(){var p=fgbProjects[fgbById('project').value]||fgbProjects.fgb;fgbById('projectLabel').textContent=p[0];fgbById('brand').textContent=p[1];fgbById('episodeLine').textContent='Episode '+fgbById('episodeNumber').value+' • '+fgbById('episodeTitle').value;fgbById('timer').textContent=fgbClock(fgbLeft);fgbById('status').textContent=fgbRunning?'Starting Soon':'Ready';fgbById('businessOut').textContent=fgbById('business').value;fgbById('locationOut').textContent=fgbById('location').value;fgbById('contactOut').textContent=fgbById('contact').value;fgbById('promotionOut').textContent=fgbById('promotion').value;var pct=((fgbTotal-fgbLeft)/fgbTotal)*100;fgbById('progress').firstElementChild.style.width=Math.max(0,Math.min(100,pct))+'%';var keys=Object.keys(fgbVisible);for(var i=0;i<keys.length;i++){fgbShow(keys[i],fgbVisible[keys[i]])}}
