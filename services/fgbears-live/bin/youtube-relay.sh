...
# FFmpeg acts as the local RTMP server for the primary encoder, then remuxes the
# already-encoded H.264/AAC packets into a normal FLV output for YouTube. There
# is no second video/audio encode here; -c copy preserves the primary encode.
exec ffmpeg \
