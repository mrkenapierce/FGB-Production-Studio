...
YOUTUBE_SERVICE=${FGB_YOUTUBE_SERVICE:-fgbears-youtube-copy-relay.service}
YOUTUBE_WARNING_FILE=${YOUTUBE_WARNING_FILE:-$HEALTH_STATE_DIR/youtube-copy-warning}
...
run_news_refresh() {
  ...
  if ! runuser -u fgbears -- env FGB_BEARS_NEWS_FEED_PATH="$NEWS_LOCAL_FEED" FGB_BEARS_NEWS_STATUS_PATH="$NEWS_REFRESH_STATUS" python3 "$NEWS_REFRESH_BIN" --interval-seconds "$NEWS_REFRESH_INTERVAL_SECONDS"; then
...
guarded_restart() {
  ...
  now=$(date +%s); cutoff=$((now - RESTART_WINDOW_SECONDS)); temporary="${RESTART_HISTORY_FILE}.partial"
  if [[ -s "$RESTART_HISTORY_FILE" ]]; then awk -v cutoff="$cutoff" '$1 >= cutoff { print $1 }' "$RESTART_HISTORY_FILE" > "$temporary"; else : > "$temporary"; fi
...
recover_youtube_destination() {
  ...
  logger -t fgbears-live-health "YouTube copy relay is inactive; restarting only ${YOUTUBE_SERVICE}."
  systemctl reset-failed "$YOUTUBE_SERVICE" || true
  systemctl restart "$YOUTUBE_SERVICE" || true
  for _ in {1..20}; do
    if systemctl is-active --quiet "$YOUTUBE_SERVICE"; then
      rm -f "$YOUTUBE_WARNING_FILE"
      logger -t fgbears-live-health "YOUTUBE_COPY_RECOVERY=RECOVERED service=${YOUTUBE_SERVICE}"
      return 0
    fi
    sleep 0.25
  done
  printf '%s service=%s recovery=failed\n' "$(date +%s)" "$YOUTUBE_SERVICE" > "$YOUTUBE_WARNING_FILE"
  logger -t fgbears-live-health "YOUTUBE_COPY_RECOVERY=FAILED service=${YOUTUBE_SERVICE}. Master left untouched."
  return 1
}
...
run_lag_check() {
  ...
  temporary="${LAG_STATUS_FILE}.partial"; printf '%s\n' "$output" > "$temporary"; mv -f "$temporary" "$LAG_STATUS_FILE"
  status=$(printf '%s\n' "$output" | sed -n 's/^OVERALL_STATUS=//p' | tail -n1)
  reason=$(printf '%s\n' "$output" | sed -n 's/^REASON=//p' | tail -n1)
  speed=$(printf '%s\n' "$output" | sed -n 's/^INTERVAL_SPEED=//p' | tail -n1)
  if [[ "$reason" == ENCODER_BELOW_REALTIME ]]; then
    printf '%s speed=%s threshold=%s\n' "$(date +%s)" "${speed:-NA}" "$LAG_WARN_SPEED" > "$LAG_WARNING_FILE"
    logger -t fgbears-live-health "Encoder lag warning: interval_speed=${speed:-NA}x threshold=${LAG_WARN_SPEED}x. Stream left running."
  elif [[ "$status" == OK ]]; then rm -f "$LAG_WARNING_FILE"; fi
}
...
run_audio_check() {
  ...
  else rm -f "$AUDIO_HEALTH_WARNING_FILE"; fi
}
...
if ! systemctl is-active --quiet fgbears-live.service; then guarded_restart SERVICE_NOT_ACTIVE; exit 0; fi
if [[ ! -s "$PROGRESS_FILE" ]]; then guarded_restart PROGRESS_MISSING; exit 0; fi
now=$(date +%s); updated=$(stat -c %Y "$PROGRESS_FILE"); age=$((now - updated))
out_time_us=$(sed -n 's/^out_time_us=\([0-9]*\)$/\1/p' "$PROGRESS_FILE" | tail -n1)
if (( age > 90 )); then guarded_restart "PROGRESS_STALE_${age}S"; rm -f "$HEALTH_SAMPLE_FILE"; exit 0; fi
