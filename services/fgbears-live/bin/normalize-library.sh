...
# Conservative, deterministic speech mastering. This intentionally avoids
# aggressive denoising/noise gates that can pump, clip word endings, or damage
# archival episodes. Override only for a deliberately versioned future profile.
...
for filter in highpass lowpass equalizer deesser acompressor loudnorm aresample; do
  if ! grep -Eq "[[:space:]]${filter}[[:space:]]" <<<"$FILTER_LIST"; then
    echo "Required FFmpeg audio filter is unavailable: $filter" >&2
    exit 69
  fi
done
...
# Measure the mastered signal before final loudness normalization. This remains
# fully offline, so the live stream never pays the DSP/timestamp cost.
ffmpeg -hide_banner -nostdin -v info -i "$INPUT" -map 0:a:0 -vn \
  -af "${MASTERING_CHAIN},loudnorm=I=${LOUDNESS_TARGET_I}:LRA=${LOUDNESS_TARGET_LRA}:TP=${LOUDNESS_TARGET_TP}:print_format=json" \
...
if [[ "$VIDEO_SIGNATURE" == "h264|1280|720|30/1|yuv420p" ]]; then
  # Existing library media is already video-normalized. Preserve video bit-for-bit
  # and replace only the offline-mastered audio track.
...
# Verify the encoded deliverable, not merely the filter graph.
...
PROFILE_JSON=$(python3 - "$VERIFY_LOG" "$PROFILE_VERSION" "$LOUDNESS_TARGET_I" "$LOUDNESS_TARGET_LRA" "$LOUDNESS_TARGET_TP" "$LOUDNESS_TOLERANCE_LU" "$MAX_POST_TRUE_PEAK" "$MASTERING_CHAIN" <<'PY'
...
printf 'Mastered audio profile %s: %s\n' "$PROFILE_VERSION" "$OUTPUT"
printf '%s\n' "$PROFILE_JSON"
