...
# Keep one live video clock. The ad renderer's existing multipart-JPEG endpoint
# is now the sole video input, so ad/interstitial frame changes reach FFmpeg
# without adding another compositor clock. The concat playlist remains audio
# only. FFmpeg converts the 25fps MJPEG source to the 24fps YouTube output.
# Retired frozen source (never restore): -loop 1 -framerate "$OUTPUT_FPS" -i "$AD_FRAME_FILE"
ffmpeg \
