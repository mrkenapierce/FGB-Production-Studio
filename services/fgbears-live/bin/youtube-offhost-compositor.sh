...
# over SRT, overlays only the transparent RGBA question mask, re-encodes video on
# this separate worker, copies AAC audio, and publishes to YouTube RTMPS.
...
exec ffmpeg -hide_banner -nostdin -loglevel warning \
  ...
  -f rawvideo -pixel_format rgba -video_size 1280x720 -framerate 30 \
  -i "http://127.0.0.1:${MASK_PORT}/overlay.rgba" \
  -filter_complex '[0:v:0][1:v:0]overlay=0:0:format=auto:shortest=1[v]' \
  -map '[v]' -map 0:a:0 \
