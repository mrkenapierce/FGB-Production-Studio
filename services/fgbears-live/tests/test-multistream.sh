...
grep -Fq 'FACEBOOK_RELAY_ENABLED' "$ROOT/bin/start-stream.sh"
grep -Fq 'FACEBOOK_LOCAL_RTMP_BASE' "$ROOT/bin/start-stream.sh"
grep -Fq 'fgb-facebook' "$ROOT/bin/start-stream.sh"
grep -Fq 'Facebook local relay' "$ROOT/bin/start-stream.sh"
...
grep -Fq 'systemctl restart fgbears-live.service' "$ROOT/bin/configure-facebook.sh"
if grep -Fq 'systemctl restart fgbears-youtube-relay.service' "$ROOT/bin/configure-facebook.sh"; then
  echo 'Facebook configuration must not directly restart the dedicated YouTube relay.' >&2
  exit 1
fi

if grep -Fq 'FACEBOOK_' "$ROOT/bin/youtube-relay.sh" || grep -Fq 'fgb-facebook' "$ROOT/bin/youtube-relay.sh" || grep -Fq -- '-f tee' "$ROOT/bin/youtube-relay.sh"; then
...
# Generate the program once, then model the exact packet topology:
# primary tee -> YouTube local + X + Facebook local; each relay copy-remuxes.
...
ffmpeg -hide_banner -loglevel error \
  ...
  "[f=flv:flvflags=no_duration_filesize:onfail=ignore]$TMP/youtube-local.flv|[f=flv:flvflags=no_duration_filesize:onfail=ignore]$TMP/x.flv|[f=flv:flvflags=no_duration_filesize:onfail=ignore]$TMP/facebook-local.flv"
...
ffmpeg -hide_banner -loglevel error -i "$TMP/facebook-local.flv" -map 0:v:0 -map 0:a:0 -c copy -f flv -flvflags no_duration_filesize "$TMP/facebook-upstream.flv"

for output in "$TMP/youtube-local.flv" "$TMP/youtube-upstream.flv" "$TMP/x.flv" "$TMP/facebook-local.flv" "$TMP/facebook-upstream.flv"; do
  test -s "$output"
...
echo 'FGBears isolated Facebook localhost sidecar tests passed.'
