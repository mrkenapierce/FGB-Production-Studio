...
grep -Fq 'Requires=fgbears-youtube-relay.service' "$ROOT/systemd/fgbears-live.service"
...
# Prove the primary encode can fan identical H.264/AAC packets to a local
# YouTube-relay input plus secondary FLV outputs. This remains one encode.
ffmpeg -hide_banner -loglevel error \
  ...
  "[f=flv:flvflags=no_duration_filesize:onfail=abort]$TMP/youtube-relay-input.flv|[f=flv:flvflags=no_duration_filesize:onfail=ignore]$TMP/x.flv|[f=flv:flvflags=no_duration_filesize:onfail=ignore]$TMP/facebook.flv"
...
echo 'FGBears single-encode plus YouTube copy-remux relay tests passed.'
