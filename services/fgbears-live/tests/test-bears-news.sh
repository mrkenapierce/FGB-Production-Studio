...
# News uses the same stable MJPEG image-composition model as the lower crawl,
# now cadence-matched to the 30 fps master. Rawvideo remains forbidden because
# it previously created pacing lag.
...
<item><title>This is a deliberately long Chicago Bears headline that must remain complete through the source</title><source url="https://www.chicagobears.com/">ChicagoBears.com</source><category>normal</category></item>
...
PID=$!
for _ in {1..50}; do
  if curl --silent --fail --max-time 1 http://127.0.0.1:18789/healthz >"$TMP/health.json" 2>/dev/null; then break; fi
  sleep 0.1
done
...
for path in sys.argv[1:]:
    im = Image.open(path); im.load(); assert im.size == (1280, 104), (path, im.size)
...
echo 'Bears news 30fps image-renderer composition tests passed.'
