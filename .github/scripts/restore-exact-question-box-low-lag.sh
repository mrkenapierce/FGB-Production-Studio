...
# Keep the current question-phase latch/stale protection, but execute only the
...
if src.count(old) != 1:
    raise SystemExit('full-middle source block not found exactly once')
...
rollback(){
  ...
  # Safety fallback is direct YouTube relay, never the lag-producing 720p compositor.
  systemctl stop "$COMP" "$ROUTING" 2>/dev/null || true
  systemctl reset-failed "$LEGACY" || true
  systemctl start "$LEGACY" || true
  echo "EXACT360_ROLLBACK_FALLBACK=DIRECT_YOUTUBE master=$(pid "$MASTER") rumble=$(pid "$RUMBLE") legacy=$(pid "$LEGACY")"
  rm -rf "$BACKUP"
  exit "$rc"
}
trap rollback EXIT
...
for _ in $(seq 1 25); do
  if health=$(curl -fsS --max-time 2 http://127.0.0.1:8791/healthz 2>/dev/null); then
    if printf '%s' "$health" | python3 -c 'import json,sys;p=json.load(sys.stdin);assert p.get("ok") is True;assert p.get("canvas")==[640,360],p;assert p.get("sourceMaskRegion")=={"x":462,"y":104,"width":798,"height":470},p;assert p.get("maskRegion")=={"x":231,"y":52,"width":399,"height":235},p;assert p.get("executionScaling")=="proportional_downstream",p;assert p.get("fps")==30,p' 2>/dev/null; then MASK_OK=1; break; fi
  fi
  sleep 1
done
(( MASK_OK == 1 ))
