...
jq -e --arg sha "$SOURCE_SHA" '.presentation.audio.revision==5 and .presentation.audio.approvalState=="approved" and .presentation.audio.activeTrackName=="Episode 034" and .presentation.audio.sourceSha256==$sha and .presentation.audio.enabled==true and .presentation.audio.loop==true and .presentation.audio.mode=="track"' /tmp/fgb-contract.json >/dev/null
...
echo "REV5_PREFLIGHT=PASS canonical_sha256=$stage_sha"

mkdir -p "$Q"
stamp=$(date -u +%Y%m%dT%H%M%SZ)
BACKUP="$Q/$stamp-pre-rev5.m4a"
...
failed={int(x) for x in s.get('failedRevisions',[]) if str(x).isdigit()}; failed.update({4,5})
s.update(status='rolled_back',candidateRevision=5,failedRevisions=sorted(failed),message=reason,completedAt=time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()))
...
failed={int(x) for x in s.get('failedRevisions',[]) if str(x).isdigit()}; failed.add(4); failed.discard(5)
s.update(appliedRevision=5,candidateRevision=5,activeTrackId='8c5d75a7-47c8-490c-8904-0c78b0140e09',activeTrackName='Episode 034',sourceSha256=source,canonicalSha256=canon,pacingRatio=float(ratio),status='live',shadow=False,failedRevisions=sorted(failed),message='Off-host mastered Episode 034 deployed and verified',completedAt=time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()))
t=p+'.tmp'; open(t,'w').write(json.dumps(s,indent=2,sort_keys=True)+'\n'); os.replace(t,p)
PY

echo "EPISODE_034_ACTIVATION=PASS revision=5 pacing=$ratio canonical_sha256=$canonical_sha"
